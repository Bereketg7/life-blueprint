...
const RootNavigator = () => {
  ...
  const handleOnboardingComplete = (data: Partial<UserProfile> & { name?: string; email?: string }) => {
    const profile: UserProfile = {
      ...
      primaryGoal: (data.primaryGoals?.[0] ?? data.primaryGoal ?? 'general-wellness') as UserProfile['primaryGoal'],
      primaryGoals: data.primaryGoals ?? (data.primaryGoal ? [data.primaryGoal] : []),
